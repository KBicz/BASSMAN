...
if ("--multi" in argv and "-h" not in argv and '--help' not in argv) or not exists(
        f"/{homename}/{profile}/.theanorc"):
    i = 2
    ...
    if platform != "darwin":
        ...
        if distro.id() == 'debian' or distro.id() == 'mint' or 'pop' in distro.id():
            line = ("[global]\ndistro = cpu\nbase_compiledir={}\n\n[blas]\nldflags= -L/usr/lib/x86_64-linux-gnu/" +
                    "openblas-pthread/ -lopenblas".format(theano_dir))
        elif distro.id() == 'ubuntu':
            lline = ("[global]\ndistro = cpu\nbase_compiledir={}\n\n[blas]\nldflags= -L/usr/lib/x86_64-linux-gnu/" +
                     "openblas-pthread/ -lblas".format(theano_dir))
        else:
            line = "[global]\ndistro = cpu\nbase_compiledir={}\n\n[blas]\nldflags= -L/usr/lib64/ -lopenblas".format(
                theano_dir)
        del distro
    else:
        line = "[global]\ndistro = cpu\nbase_compiledir={}".format(theano_dir)
    with open("/{}/{}/.theanorc".format(homename, profile), 'w') as of:
        of.write(line)
    del line
...
if starry.__version__ == "1.0.0":
    import exoplanet as pmx
else:
...
def helpf():
    print("\n  Program bassman.py for Linux and MacOS written by K. Bicz, ver. of Jun. 25, 2024.")
    ...
    print("         [-chi=float] [-logp=float] [-temp=float] [-terr=float] [-nth=int] [--pran] [--magfield] [--filter]")
    print("         [--init] [--diff] [--force] [--nodisp] [--sample] [--noadap] [--fulltime] [--nosphere] [--nochi] ")
    print("         [--check] [--ps] [--fscale] [--pcom] [--liveplot] [--modelonly] [--multi]")
    print()
    ...
    print("                --magfield  : print analitical estimations of spot(s) magnetic fields from Valio 2020")
    ...
    print("                --multi     : use multiple theano instances for parallel computing in different windows.")
    print()
    exit()


def save_gif(name, image, time, speed, tic, timedata, fluxdata, fluxmodel, starttbjd, vmin, vmax, sphere=False,
             aitoff=False):
    arr = []
    if sphere:
        nam = 'sphere'
    ...
    for i in progress_bar(range(image.shape[0]), desc="Preparing {} gif".format(nam)):
        arr.append(image[i])
        if sphere:
            fig, ax = plt.subplots(nrows=2, ncols=1, sharex=False, sharey=False, gridspec_kw={"height_ratios": [2, 1]},
                                   figsize=(8, 9))
            FigureCanvas(fig)
            sto = (i + 1) * nparts
            ...
            ax.margins(0)
        fig.canvas.draw()
        arr[i] = np.asarray(fig.canvas.buffer_rgba())
...
def print_results(map_soln, nspots, omega_eq, tns, ampl):
    ...
    for i in range(1, nspots + 1):
        ...
        data[i, 4] = "{:.4f}".format(
            360 / (omega_eq * (1 - map_soln['alpha'] * np.sin(map_soln['lat{:d}'.format(i)] * np.pi / 180) ** 2)))
        try:
            data[i, 5] = str(int(round(tns['temp{:d}'.format(i)]))) + " ± " + str(
                int(round(tns['temp{:d}err'.format(i)])))
        except:
            data[i, 5] = "NaN"
        ...
        try:
            data[i, 9] = "{:.2e}".format(tns['flux{:d}'.format(i)])
        except:
            data[i, 9] = "NaN"
        data[i, 10] = " {:.5f}".format(np.abs(map_soln['amp{:d}'.format(i)]) * map_soln['scale'] * 100 / ampl)
    print(DataFrame(data, tabtab,
                    ["Amplitude", "Sigma", "Latitude", "Longitude", u"\u03B1" + "={:.4f}".format(
                        map_soln['alpha']), "Temperature", "Size", "Radius", 'Diameter', "Flux", " Contrast"]))
...
def tempandsiz(map_soln, B, ampl, nspots):
    ...
    for i in range(1, nspots + 1):
        ...
        sec.add_spot(amp=map_soln['amp{:d}'.format(i)] * map_soln['scale'], sigma=map_soln['sigma{:d}'.format(i)],
                     lat=0.0, lon=0.0)
        sec[1] = B["u"][0]
        sec[2] = B["u"][1]
        ...
        if map_soln['amp{:d}'.format(i)] <= 0:
            msig = np.mean(mapa[sta1:sto1, sta2:sto2][(
                np.where((mapa[sta1:sto1, sta2:sto2] < np.median(mapa)) & (mapa[sta1:sto1, sta2:sto2] >= 0)))])
            terr = np.std(mapa[sta1:sto1, sta2:sto2][(
                np.where((mapa[sta1:sto1, sta2:sto2] < np.median(mapa)) & (mapa[sta1:sto1, sta2:sto2] >= 0)))])
        else:
            msig = np.mean(mapa[sta1:sto1, sta2:sto2][(
                np.where((mapa[sta1:sto1, sta2:sto2] > np.median(mapa)) & (mapa[sta1:sto1, sta2:sto2] >= 0)))])
        ...
        if B['temp'] != 'nan':
            ...
            tns['flux{:d}'.format(i)] = Stefan_Boltzmann * tns['temp{:d}'.format(i)] ** 4 * tns[
                'psize{:d}'.format(i)] / 100
            tns['flux{:d}err'.format(i)] = Stefan_Boltzmann * tns['temp{:d}err'.format(i)] ** 4 * tns[
                'psize{:d}'.format(i)] / 100
        else:
...
def magfield(tstar, tstarerr, tspot):
    a, b = ufloat(-1.08, 0.04) * 1e-8, ufloat(0.98, 0.03)
    star = ufloat(tstar, tstarerr)

    return ((tspot - b * star) / (star * a)) ** 0.5
    ...
    for i in range(len(lines)):
        ...
        try:
            if "Spot" in line[0]:
                ...
                scale = float(line[3])
            if "Amplitude" in line and "#" not in line:
                if "=" in line[5]:
            ...
            pass
    try:
        return np.array(amplw), np.array(sigms), np.array(lats), np.array(lons), alpha, scale
    except:
        return np.array(amplw), np.array(sigms), np.array(lats), np.array(lons), alpha, 1 / len(amplw)
...
def checkmodel(amps, sizs, lats, lons, alpha, ampran, sizran, latran, lonran, alphatab, nspots):
    ...
    for val, rang, i in zip(values, ranges, ["amplitudes", "sizes", "latitutdes", 'longitudes']):
        if np.max(val) > np.max(rang) or np.min(val) < np.min(rang) or alpha > np.max(alphatab) or alpha < np.min(
                alphatab):
            print("\n\a# Error! One of the {} is not in the range of given parameters!".format(i))
            exit()


def recreate_sspots(params, nspots, prec, dprec, prs, omb, obm, def_inclination, gv, ylm, stara, alphatab, alu, slu,
                    lalu, lolu, adap, mprod, savectrl, save, force, displayctrl, sample, noadap, finc, gsp, nstep,
                    diffctrl, filterctrl, fulltime, nosphere, command, modelfile, modelctrl, staractrl, tbjd, tbjdctrl,
                    initctrl, ncores, nchains, chiv, logpv, chivctrl, logpvctrl, nochi, temper, temperctrl, check, pcom,
                    ps, fscale, pran, falpha, falphactrl, publication, liveplot, terrctrl, terr, modelonly, nth,
                    magfieldctrl):
    if publication:
    ...
    if tbjdctrl:
        ...
        if (tbjd < t[0] + data['mintim'] or tbjd > t[-1] + data['mintim']):
            print(
                "% Warning! Given TBJD = {} is not in observational interval {}-{}.".format(tbjd, t[0] +
                                                                                            data['mintim'], t[-1] +
                                                                                            data['mintim']))
    ...
    if diffctrl:
        t_ani = np.linspace(tbjd, tbjd + 3 * B['prot'], 100)
    else:
        t_ani = np.linspace(tbjd, tbjd + B['prot'], 100)

    flux = data["flux"][::nth]
    sigma = data['sigma'][::nth]
    if not staractrl:
        ampl = np.mean(flux) + 1.4 * np.std(flux)
    else:
        ampl = stara
    ...
    while (m > prec or abs(1 - round(bbb, 3)) > omb or abs(
            1 - round(pers, 2)) > prs or mf < ampl - obm or mf > ampl + obm or amplctrl == False):
        gc.collect()
        print()
        if iterat > nstep and not force:
            ...
            print("\a% Model for {} spots does not converge, increasing number of spots to {}.".format(nspots - 1,
                                                                                                       nspots))
        ...
        if ((iterat > nstep + 20 and not diffctrl) or (iterat > 2 * (nstep + 20) and diffctrl)) and force:
            ...
            if len(chisqtab) > 0:
                print("> Recreating model with best " + u"\u03A7" + "^2.")
                whchisq = int(np.array(chisqtab).argmin())
                amps, sigms, lats, lons, alphaa = amptab[whchisq], sigmatab[whchisq], lattab[whchisq], lontab[whchisq], \
                    alphaatab[whchisq]
                modelctrl = True
            else:
                exit()
        ...
        if m == 1000000 * precd:
            print("# Error! Infinite model!")
        else:
            if ((len(ms) >= adap and abs(m - prec) >= dprec and round(min(ms) / prec) <= mprod and not noadap) and (
                    (prec == precd) or (prec != precd and min(ms) + dprec < prec))) and not logpvctrl and not chivctrl:
                if min(ms) + dprec > prec:
                    prec = min(ms) + dprec
                else:
                    prec += dprec
                ...
                if abs(1 - round(bbb, 3)) > omb: print(
                    "# |1-b| = {} but has to be less equal {}.".format(abs(1 - round(bbb, 3)), omb))
                if abs(1 - round(pers, 2)) > prs: print(
                    "# |1-pearson| = {} but has to be less equal {}.".format(abs(1 - round(pers, 2)), prs))
                if mf < ampl - obm or mf > ampl + obm: print(
                    "# Flux_model_max = {} but has to be between {}-{}.".format(mf, ampl - obm, ampl + obm))
                if False in spots_bools:
                    ...
                    spots_bools = [True for _ in range(nspots)]
            if logpvctrl and logpv > logp and iterat > 2: print(
                "# Error! Desired logp has not been achieved! logp has to be greater than {}.".format(logpv))
            if chivctrl and chiv < chisqselnc and iterat > 2: print(
                "# Error! Desired chisq has not been achieved! chisq has to be less than {}.".format(chiv))
        ...
        with pm.Model() as model:
            ...
            if not modelctrl:
                ...
                amps, sigms, lats, lons = list(ampso.copy()), list(sigmso.copy()), list(latso.copy()), list(
                    lonso.copy())
                ...
                lons = [get_random(lor[0], lor[1]) for _ in range(nspots)]
            for i in range(1, nspots + 1):
                sspots['amp{:d}'.format(i)] = pm.Uniform("amp{:d}".format(i), lower=alu[0], upper=alu[1],
                                                         testval=amps[i - 1])
                sspots['sigma{:d}'.format(i)] = pm.Uniform("sigma{:d}".format(i), lower=slu[0], upper=slu[1],
                                                           testval=sigms[i - 1])
                sspots['lat{:d}'.format(i)] = pm.Uniform("lat{:d}".format(i), lower=lalu[0], upper=lalu[1],
                                                         testval=lats[i - 1])
                sspots['lon{:d}'.format(i)] = pm.Uniform("lon{:d}".format(i), lower=lolu[0], upper=lolu[1],
                                                         testval=lons[i - 1])

                sec[i - 1] = starry.Map(ydeg=B["ydeg"][0], udeg=B["udeg"][0], inc=B['inc'], amp=ampl)
                sec[i - 1].add_spot(amp=sspots['amp{:d}'.format(i)], sigma=sspots['sigma{:d}'.format(i)],
                                    lat=sspots['lat{:d}'.format(i)], lon=sspots['lon{:d}'.format(i)])
                sec[i - 1][1] = B["u"][0]
                sec[i - 1][2] = B["u"][1]
            ...
            gc.collect()
            if starry.__version__ == "1.0.0":
                map_soln = pmx.optimize(vars=[*sspots.values(), *scalealpha], options={"maxiter": 10000},
            ...
            if str(map_soln['flux_model'][0]) != "nan":
                for i in range(1, nspots + 1):
                    if starry.__version__ == "1.0.0":
                        map_soln = pmx.optimize(vars=[sspots['amp{:d}'.format(i)], sspots['sigma{:d}'.format(i)],
                                                      sspots['lat{:d}'.format(i)], sspots['lon{:d}'.format(i)],
                                                      *scalealpha], start=map_soln, options={"maxiter": 10000})
                    else:
                        map_soln = pmx.optimize(vars=[sspots['amp{:d}'.format(i)], sspots['sigma{:d}'.format(i)],
                                                      sspots['lat{:d}'.format(i)], sspots['lon{:d}'.format(i)],
                                                      *scalealpha], start=map_soln, maxeval=10000)
                    gc.collect()
                if starry.__version__ == "1.0.0":
                    map_soln, info = pmx.optimize(vars=[*sspots.values(), *scalealpha], start=map_soln,
                                                  return_info=True, options={"maxiter": 10000})
                else:
                    map_soln, info = pmx.optimize(vars=[*sspots.values(), *scalealpha], start=map_soln,
                                                  return_info=True, maxeval=10000)
                gc.collect()
                logp = -info.fun
            else:
                m = 'nan'
        ...
        if not fscale:
            ...
            nc = nspots * 4 + 1
        if not diffctrl:
            map_soln['alpha'] = alpha
        else:
            nc += 1
        fmchi = pmx.eval_in_model(flux_model, map_soln, model=model)
        m, bbb = abs(np.polyfit(t, flux / fmchi, 1))
        mf, chisqsel, chisqselnc = max(fmchi), np.sum((flux - fmchi) ** 2 / (sigma) ** 2) / len(fmchi), np.sum(
            (flux - fmchi) ** 2 / (sigma) ** 2) / (len(fmchi) - nc)
        print(u"\u03A7" + "^2/(N-{:d}) = {}".format(nc, chisqselnc))
        ...
        if check:
            with model:
                ...
                for n in range(1, nspots + 1):
                    ...
                    teststar[n - 1].add_spot(amp=map_soln['amp{:d}'.format(n)] * map_soln['scale'],
                                             sigma=map_soln['sigma{:d}'.format(n)], lat=map_soln['lat{:d}'.format(n)],
                                             lon=map_soln['lon{:d}'.format(n)])
                    teststar[n - 1][1] = B["u"][0]
                    teststar[n - 1][2] = B["u"][1]

                    testomega[n - 1] = omega_eq * (
                            1 - map_soln['alpha'] * np.sin(sspots['lat{:d}'.format(n)] * np.pi / 180) ** 2)
                    flux_model_test = pmx.eval_in_model(teststar[n - 1].flux(theta=testomega[n - 1] * t), map_soln,
                                                        model=model)
        ...
        if ((((iterat > nstep + 20 and not diffctrl) or (iterat > 2 * (nstep + 20) and diffctrl)) and force) or (
                logp >= logpv and logpvctrl) or (chisqselnc <= chiv and chivctrl)) and amplctrl == True: break
    ...
    for i in range(1, nspots + 1):
        if map_soln["sigma{:d}".format(i)] >= 0.5:
            print("\a% Warning! Size of spot no. {:d} is very big! Model may not be correct!".format(i))
        elif map_soln["sigma{:d}".format(i)] <= 0.001:
            print("\a% Warning! Size of spot no. {:d} is very small! Model may not be correct!".format(i))
    ...
    if not diffctrl:
        whfl = np.where(
            ((data['timeorig'] - data['mintim'] >= t[0]) & (data['timeorig'] - data['mintim'] <= t[0] + B['prot'])))[0]
    else:
        whfl = np.where(((data['timeorig'] - data['mintim'] >= t[0]) & (
                data['timeorig'] - data['mintim'] <= t[0] + 3 * B['prot'])))[0]

    fluxsum = 0
    for i in range(1, nspots + 1):
        try:
            fluxsum += tns['flux{:d}'.format(i)]
        except:
            pass

    if modelonly:
        np.savez(savename("model_results.npz"), B=B, map_soln=map_soln, nspots=nspots, nc=4 * nspots, addon=addon,
                 omega_eq=360 / B['prot'], fulltime=fulltime, gv=gv, ampl=ampl, publication=publication,
                 amplctrl=amplctrl, terrctrl=terrctrl, terr=terr, tns=tns, displayctrl=displayctrl, tic=tic,
                 nochi=nochi, chisqselnc=chisqselnc, fmchi=fmchi, command=command, mf=mf, logp=logp, fluxsum=fluxsum,
                 pers=pers, m=m, bbb=bbb, whfl=whfl, t_ani=t_ani, tbjdctrl=tbjdctrl, gsp=gsp, filterctrl=filterctrl,
                 tbjd=tbjd, tbjdnam=tbjdnam)
        exit()

    secimshow = starry.Map(ydeg=B["ydeg"][0], udeg=B["udeg"][0], inc=B['inc'], amp=ampl, alpha=map_soln['alpha'])
    for i in range(1, nspots + 1):
        secimshow.add_spot(amp=map_soln['amp{:d}'.format(i)] * map_soln['scale'], sigma=map_soln['sigma{:d}'.format(i)],
                           lat=map_soln['lat{:d}'.format(i)], lon=map_soln['lon{:d}'.format(i)])
    secimshow[1] = B["u"][0]
    secimshow[2] = B["u"][1]
    gc.collect()
    ...
    for i in range(1, nspots + 1):
        sec[i - 1] = starry.Map(ydeg=B["ydeg"][0], udeg=B["udeg"][0], inc=B['inc'], amp=ampl)
        sec[i - 1].add_spot(amp=map_soln['amp{:d}'.format(i)], sigma=map_soln['sigma{:d}'.format(i)],
                            lat=map_soln['lat{:d}'.format(i)], lon=map_soln['lon{:d}'.format(i)])
        sec[i - 1][1] = B["u"][0]
        sec[i - 1][2] = B["u"][1]
        ...
        if isgap(t, gv) or (isgap(data['ort'], gv) and fulltime): flux_model2[i - 1] = sec[i - 1].flux(
            theta=omega[i - 1] * t2)
        flux_model3[i - 1] = sec[i - 1].flux(theta=omega[i - 1] * ttt)
        gc.collect()

    if isgap(t, gv) or (isgap(data['ort'], gv) and fulltime): flux_model2 = map_soln['scale'] * pm.math.sum(flux_model2,
                                                                                                            axis=0)
    ...
    if pran:
        if amplctrl:
            print("% Assumed star amplitude = {}".format(ampl))
        else:
            print("% Estimated star amplitude = {}".format(ampl))
        if terrctrl:
            terror = terr * (0.751 - 0.0000716 * float(B['temp']))
            serror = np.sqrt((400 * (1 - min(flux_model_orig) / ampl) * atemp ** 3 / (
                    float(B['temp']) ** 4 * (1 - (atemp / float(B['temp'])) ** 4) ** 2) * terror) ** 2 + (
                                     400 * (1 - min(flux_model_orig) / ampl) * atemp ** 4 / (
                                     float(B['temp']) ** 5 * (
                                     1 - (atemp / float(B['temp'])) ** 4) ** 2) * terr) ** 2)
            ...
            print("% Estimated mean temperature of spots = {:.0f} ± {:.0f} K\n".format(
                (fs / (aspots * 0.01 * Stefan_Boltzmann)) ** 0.25,
                (np.sqrt(terrmean) / (aspots * 0.01 * Stefan_Boltzmann)) ** 0.25))
        else:
    ...
    if magfieldctrl:
        if nspots == 1:
            print("% Magnetic fields of the spot:")
        else:
            print("% Magnetic fields of the spots:")
        for iii in range(1, nspots + 1):
            magf = magfield(float(B['temp']), terr, ufloat(int(round(tns['temp{:d}'.format(iii)])),
                                                           int(round(tns['temp{:d}err'.format(iii)]))))
            print(f"  Spot{iii:d}: {magf.nominal_value:.0f} ± {magf.std_dev:.0f} Gs")
        print()
        del magf
        gc.collect()
    ...
    with model:
        for i in range(1, nspots + 1):
            components["Spot {:d}".format(i)] = pmx.eval_in_model(sec[i - 1].flux(theta=omega[i - 1] * tc), map_soln,
                                                                  model=model) * map_soln['scale']
            if map_soln['amp{:d}'.format(i)] < 0:
                maxcom.append(np.max(components["Spot {:d}".format(i)]))
            else:
                maxcom.append(np.min(components["Spot {:d}".format(i)]))
            gc.collect()
    ...
    if savectrl:
        save2 = save.split(".")
        if not fulltime:
            sfil = savename("{}_components_inc{:d}_n{:d}{}.txt".format(save2[0], int(B['inc']), nspots, addon))
        else:
            sfil = savename("{}_components_all_inc{:d}_n{:d}{}.txt".format(save2[0], int(B['inc']), nspots, addon))
    ...
    if savectrl:
        ...
        print("# Estimated mean temperature of spots = {:.0f} K".format(
            (fs / (aspots * 0.01 * Stefan_Boltzmann)) ** 0.25))
        print("# Estimated spotedness of star = {:.2f} %".format(aspots))
        if not terrctrl:
            print("# Analytical mean temperature of spots = {:.0f} K".format(atemp))
        ...
        if not fulltime:
            np.savetxt(sfil, np.transpose(
                [data['timeorig'], data['fluxorig'], data['errorig'], (flux_model_orig - 1.) * 1000]))
        else:
            np.savetxt(sfil, np.transpose([data['ort'] + data['mintim'], (data['orf'] - 1.) * 1000, data['ore'] * 1000,
                                           (flux_model4 - 1.) * 1000]))
        ...
        if not fulltime:
            np.savetxt(sfil, np.transpose(
                [data['timeorig'], data['fluxorig'] - ((flux_model_orig - 1.) * 1000), data['errorig'],
                 np.ones(len(data['timeorig'])) * 2137]))
        else:
            np.savetxt(sfil, np.transpose(
                [data['ort'] + data['mintim'], ((data['orf'] - 1.) * 1000) - ((flux_model4 - 1.) * 1000),
                 data['ore'] * 1000, np.ones(len(data['ort'])) * 2137]))

        gc.collect()
        if fulltime:
            fnamelogp = "{}_logp_all_n{:d}{}.txt".format(save2[0], nspots, addon)
        else:
            fnamelogp = "{}_logp_n{:d}{}.txt".format(save2[0], nspots, addon)
        if exists(fnamelogp):
            mode = 'a'
        else:
            mode = 'w'
        with open(fnamelogp, mode) as of:
            if mode == "w":
                ...
                of.write(string)
            string = "\n        {:d}          {:.5f}   {:.5f}     {:.5f}     {:.5f}      ".format(int(B['inc']), logp,
                                                                                                  mf, chisqselnc,
                                                                                                  chisq / (len(data[
                                                                                                                   'timeorig']) - nc))
            ...
            for nsp in range(1, nspots + 1):
                try:
                    string += "   {:d}   {:d}   {:.5f}".format(int(round(tns['temp{:d}'.format(nsp)])),
                                                               int(round(tns['temp{:d}err'.format(nsp)])),
                                                               tns['psize{:d}'.format(nsp)])
                except:
    ...
    if displayctrl or savectrl:
        ...
        if fulltime:
            ...
            lll = "Model"
        if isgap(t, gv) or (isgap(data['ort'], gv) and fulltime): ax0.plot(t2 + data['mintim'],
                                                                           pmx.eval_in_model(flux_model2, map_soln,
                                                                                             model=model), marker='.',
                                                                           color="limegreen", label="Missing", ms=1,
                                                                           linestyle='')
        ...
        if initctrl: ax0.plot(t + data['mintim'], flux_model_guess, marker='.', color="dodgerblue", alpha=0.6,
                              label="Initial model", ms=1, linestyle='')
        if not nochi:
            ax0.plot([], [], ' ', label=r"$\chi^2_{\,\,sel}$/(N-" + str(nc) + r") = {}".format(chisqselnc))
            ax0.plot([], [], ' ',
                     label=r"$\chi^2_{\,\,flare}$" + r"/(N-{:d}) = {}".format(nc, chisq / (len(data['timeorig']) - nc)))
            if fulltime:
                ax0.plot([], [], ' ', label=r"$\chi^2_{\,\,full}$/(N-" + str(nc) + r") = " + str(
                    chisqfull / (len(data['orf']) - nc)))
        ...
        if not publication:
            ax0.set_title("TIC{}".format(tic), weight='bold')
        else:
        ...
        if savectrl:
            save2 = save.split(".")
            if not fulltime:
                save2 = savename("{}_flux_inc{:d}_n{:d}{}.{}".format(save2[0], int(B['inc']), nspots, addon, save2[1]))
            else:
                save2 = savename(
                    "{}_flux_all_inc{:d}_n{:d}{}.{}".format(save2[0], int(B['inc']), nspots, addon, save2[1]))
            plt.savefig(save2)
        ...
        for i in range(len(components.keys())):
            ax1.plot(tc + data['mintim'], components["Spot {}".format(i + 1)] - maxcom[i],
                     "C{}".format(cnums[i % len(cnums)]), marker='.', linestyle='', label="Spot {}".format(i + 1), ms=1)
        if fulltime:
            ax0.plot(data['ort'] + data['mintim'], data['orf'], "k.", ms=2, label="Data")
            lll = "Model for selected interval"
        else:
        ...
        if savectrl:
            ...
            if not fulltime:
                save2 = savename(
                    "{}_components_inc{:d}_n{:d}{}.{}".format(save2[0], int(B['inc']), nspots, addon, save2[1]))
            else:
                save2 = savename(
                    "{}_components_all_inc{:d}_n{:d}{}.{}".format(save2[0], int(B['inc']), nspots, addon, save2[1]))
            plt.savefig(save2)
            if not displayctrl: plt.close(1)
        if displayctrl: plt.show()
        ...
        if fulltime:
            ...
            plt.plot(data['ort'] + data['mintim'], flux_model4, marker=".", color='C3', label="Light curve model",
                     ms=msc, linestyle='')
            ...
            plt.plot(data['timeorig'], flux_model_orig, marker=".", color='C3', label="Light curve model", ms=msc,
                     linestyle='')
            tcon = data['timeorig']
            if np.max(data['fluxorig'] / 1000 + 1) > 1.02 * mf: plt.ylim(top=1.02 * mf)
        mfcon = np.array([mf for _ in tcon])
        if nspots != 1:
            for i in range(len(components.keys())): plt.plot(tcon,
                                                             mfcon + (components["Spot {}".format(i + 1)] - maxcom[i]),
                                                             "C{}".format(cnums[i % len(cnums)]), marker='.',
                                                             label="Spot {}".format(i + 1), ms=msc, linestyle='')
        plt.gca().get_xaxis().set_major_formatter(ScalarFormatter(useOffset=False))
        plt.gca().get_yaxis().set_major_formatter(ScalarFormatter(useOffset=False))
        ...
        if not publication:
            plt.title("TIC{}".format(tic), weight='bold')
        else:
        ...
        if savectrl:
            ...
            if not fulltime:
                save2 = savename(
                    "{}_contribution_inc{:d}_n{:d}{}.{}".format(save2[0], int(B['inc']), nspots, addon, save2[1]))
            else:
                save2 = savename(
                    "{}_contribution_all_inc{:d}_n{:d}{}.{}".format(save2[0], int(B['inc']), nspots, addon, save2[1]))
            plt.savefig(save2)
            if not displayctrl: plt.close(1)
        if displayctrl: plt.show()
        ...
        rec[2] = B["u"][1]
        for i in range(1, nspots + 1):
            rec.add_spot(amp=map_soln['amp{:d}'.format(i)] * map_soln['scale'], sigma=map_soln['sigma{:d}'.format(i)],
                         lat=map_soln['lat{:d}'.format(i)], lon=map_soln['lon{:d}'.format(i)])
        if displayctrl:
        ...
        if savectrl:
            save2 = save.split(".")
            save2 = savename(
                "{}_anim_sphere_grid_inc{:d}_n{:d}{}.{}".format(save2[0], int(B['inc']), nspots, addon, 'gif'))
            ...
            save_gif(sfil, image, t_ani, gsp, tic, data['timeorig'][whfl] - data['mintim'],
                     (data['fluxorig'][whfl] / 1000 + 1), flux_model_orig[whfl], data['timeorig'][whfl][0],
                     np.min(imageshow), np.max(imageshow))
            sfil = savename("{}_anim_aitoff_inc{:d}_n{:d}{}.gif".format(save2[0], int(B['inc']), nspots, addon))
            save_gif(sfil, image, t_ani, gsp, tic, data['timeorig'][whfl] - data['mintim'],
                     (data['fluxorig'][whfl] / 1000 + 1), flux_model_orig[whfl], data['timeorig'][whfl][0],
                     np.min(imageshow), np.max(imageshow), aitoff=True)
    ...
    if savectrl and not nosphere:
        tsph = np.linspace((data['timeorig'][whfl][0] - data['mintim']) / B['prot'] * 360,
                           (data['timeorig'][whfl][-1] - data['mintim']) / B['prot'] * 360, 50)
        ...
        save_gif(sfil, imagesphere, t_ani, gsp * 2, tic, data['timeorig'][whfl] - data['mintim'],
                 (data['fluxorig'][whfl] / 1000 + 1), flux_model_orig[whfl], data['timeorig'][whfl][0], vmin, vmax,
                 sphere=True)
        del imagesphere
    ...
    if sample:
        ...
        with model:
            if platform != 'darwin':
                trace = pm.sample(tune=1000, draws=2500, start=map_soln, chains=nchains, cores=ncores,
                                  return_inferencedata=False, target_accept=0.9)
            else:
                import multiprocessing as mp
                trace = pm.sample(tune=1000, draws=2500, start=map_soln, chains=1, cores=ncores,
                                  mp_ctx=mp.get_context("fork"), return_inferencedata=False, target_accept=0.9)
        ...
        if savectrl or displayctrl:
            for i in range(1, nspots + 1):
                truths_ms = [map_soln['amp{:d}'.format(i)], map_soln['sigma{:d}'.format(i)],
                             map_soln['lat{:d}'.format(i)], map_soln['lon{:d}'.format(i)]]
                samples = pm.trace_to_dataframe(trace, varnames=['amp{:d}'.format(i), 'sigma{:d}'.format(i),
                                                                 'lat{:d}'.format(i), 'lon{:d}'.format(i),
                                                                 *scalealphanames])
                ...
                if savectrl:
                    save2 = save.split(".")
                    save2 = savename(
                        "{}_corner_inc{:d}_n{:d}_spot{}{}.{}".format(save2[0], int(B['inc']), nspots, i, addon,
                                                                     save2[1]))
                    plt.savefig(save2)
                    if not displayctrl: plt.close(1)
                if displayctrl: plt.show()
            ...
            for i in np.random.choice(range(len(trace["flux_model"])), 24):
                ...
                label = None
            plt.plot(t + data['mintim'], np.mean(trace["flux_model"], axis=0), color='C3', linestyle='--', alpha=0.7,
                     label='Mean model')
            plt.legend(prop=legpara, numpoints=5, markerscale=markersize)
            if not publication:
                plt.title("TIC{}".format(tic), weight='bold')
            ...
            if savectrl:
                save2 = save.split(".")
                save2 = savename(
                    "{}_sample_inc{:d}_n{:d}{}.{}".format(save2[0], int(B['inc']), nspots, addon, save2[1]))
                plt.savefig(save2)
                if not displayctrl: plt.close(1)
            if displayctrl: plt.show()
            ...
            for i in range(1, nspots + 1):
                mapmu.add_spot(amp=np.mean(trace['amp{:d}'.format(i)]) * scale,
                               sigma=np.mean(trace['sigma{:d}'.format(i)]), lat=np.mean(trace['lat{:d}'.format(i)]),
                               lon=np.mean(trace['lon{:d}'.format(i)]))
            mapmu[1] = B["u"][0]
            mapmu[2] = B["u"][1]
            ...
            if savectrl:
                save2 = save.split(".")
                save2 = savename(
                    "{}_mean-map_inc{:d}_n{:d}{}.{}".format(save2[0], int(B['inc']), nspots, addon, save2[1]))
                plt.savefig(save2)
                if not displayctrl: plt.close()
            if displayctrl: plt.show()
    return map_soln
...
if __name__ == "__main__":
    ...
    adap, nstep, gsp, falpha, falphactrl, terr = 5, 30, 1, 0, False, 157.0
    ...
    tbjd, tbjdctrl, initctrl, ncores, nchains, pran, liveplot, magfieldctrl = 0, False, False, 4, 4, False, False, True
    for arg in argv:
        if "-star=" in arg:
            params = arg.split("=")[-1]
        elif "-nspots=" in arg:
            nspots = abs(str_to_num(arg.split("=")[-1], int, "nspots"))
        elif "-prec=" in arg:
            prec = abs(str_to_num(arg.split("=")[-1], float, "prec"))
        elif "-dprec=" in arg:
            dprec = abs(str_to_num(arg.split("=")[-1], float, "dprec"))
        elif "-pers=" in arg:
            prs = abs(str_to_num(arg.split("=")[-1], float, "pers"))
        elif "-omb=" in arg:
            omb = abs(str_to_num(arg.split("=")[-1], float, "omb"))
        elif "-obm=" in arg:
            obm = abs(str_to_num(arg.split("=")[-1], float, "obm"))
        elif "-finc" in arg:
            finc = True
            if "=" in arg: def_inclination = abs(str_to_num(arg.split("=")[-1], float, "def_inclination"))
        elif "-gv=" in arg:
            gv = abs(str_to_num(arg.split("=")[-1], float, "gv"))
        elif "-ylm=" in arg:
            ...
            stara, staractrl = abs(str_to_num(arg.split("=")[-1], float, "ampl")), True
        elif "-almin=" in arg:
            alpha[0] = abs(str_to_num(arg.split("=")[-1], float, "almin"))
        elif "-almax=" in arg:
            alpha[1] = abs(str_to_num(arg.split("=")[-1], float, "almax"))
        elif "-save" in arg:
            savectrl = True
            if '=' in arg and arg[-1] != '=': save = arg.split("=")[-1]
        elif "-adap=" in arg:
            adap = abs(str_to_num(arg.split("=")[-1], int, "adap"))
        elif "-nstep=" in arg:
            nstep = str_to_num(arg.split("=")[-1], int, "nstep")
        elif "-mprod=" in arg:
            mprod = abs(str_to_num(arg.split("=")[-1], float, "mprod"))
        elif "-amin=" in arg:
            alu[0] = str_to_num(arg.split("=")[-1], float, "amin")
        elif "-amax=" in arg:
            alu[1] = str_to_num(arg.split("=")[-1], float, "amax")
        elif "-smin=" in arg:
            ...
            gsp = str_to_num(arg.split("=")[-1], int, "gsp")
        elif "-model" in arg:
            modelctrl = True
            if "=" in arg and arg[-1] != '=': modelfile = arg.split("=")[1]
        elif "-falpha=" in arg:
            falpha, falphactrl = abs(str_to_num(arg.split("=")[-1], float, "falpha")), True
        elif "-tbjd=" in arg:
            tbjd, tbjdctrl = str_to_num(arg.split("=")[-1], float, "tbjd"), True
        elif '-ncores=' in arg:
            ncores = abs(str_to_num(arg.split("=")[-1], int, "ncores"))
        elif '-nchains=' in arg:
            nchains = abs(str_to_num(arg.split("=")[-1], int, "nchains"))
        elif "-chi=" in arg:
            chiv, chivctrl = abs(str_to_num(arg.split("=")[-1], float, "chi")), True
        elif "-logp=" in arg:
            logpv, logpvctrl = str_to_num(arg.split("=")[-1], float, "logpv"), True
        elif "-temp=" in arg:
            temper, temperctrl = abs(str_to_num(arg.split("=")[-1], float, "temperature")), True
        elif "-terr=" in arg:
            terr, terrctrl = abs(str_to_num(arg.split("=")[-1], float, "temperature error")), True
        elif "-nth=" in arg:
            nth = abs(str_to_num(arg.split("=")[-1], int, "nth"))
        elif arg == "--pran":
            pran = True
        elif arg == "--filter":
            filterctrl = True
            ...
            magfieldctrl = True
        elif (("-h" in arg and len(arg) == 2) or arg == "--help"):
            helpf()
    ...
    try:
        recreate_sspots(params, nspots, prec, dprec, prs, omb, obm, def_inclination, gv, ylm, stara, alpha, alu, slu,
                        lalu, lolu, adap, mprod, savectrl, save, force, displayctrl, sample, noadap, finc, gsp, nstep,
                        diffctrl, filterctrl, fulltime, nosphere, command, modelfile, modelctrl, staractrl, tbjd,
                        tbjdctrl, initctrl, ncores, nchains, chiv, logpv, chivctrl, logpvctrl, nochi, temper,
                        temperctrl, check, pcom, ps, fscale, pran, falpha, falphactrl, publication, liveplot, terrctrl,
                        terr, modelonly, nth, magfieldctrl)
    except KeyboardInterrupt:
        if '--mulit' in argv:
            system("rm -rf {}".format(theano_dir))
